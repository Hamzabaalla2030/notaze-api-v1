...
async function downloadWeibo(url, basePath = 'resultdownload_preniv') {
  ...
  try {
    const response = await axios.get(`${getApi.weibo}${encodeURIComponent(url)}`, {
      timeout: 30000,
      headers: {
    ...
    if (selectedDownload === 'all') {
      ...
      if (hasVideo) {
        ...
        const filename = `weibo_video_${Date.now()}.mp4`;
      ...
      if (hasImages) {
        for (let i = 0; i < data.data.original.atlas.length; i++) {
          ...
          const filename = `weibo_image_${Date.now()}_${i}.jpg`;
      ...
      if (selectedDownload.type === 'video') {
        ...
        const filename = `weibo_image_${Date.now()}.jpg`;
        await downloadFile(selectedDownload.url, filename, downloadSpinner, basePath);
      }
    }
  } catch (error) {
    spinner.fail(chalk.red(' Error fetching Weibo media'));
